...
for version in $versions_to_build; do
    ...
    [ -z "${PULL:-}" ] || docker pull "buildscale/presto-dev:$version"
    docker build -t "buildscale/presto-dev:$version" --build-arg PRESTO_VERSION="$version" $no_cache .
    [ -n "${NOPUSH:-}" ] || docker push "buildscale/presto-dev:$version"
    ...
    if [ $count -gt 1 ] && [ -z "${NODELETE:-}" ]; then
        docker rmi "buildscale/presto-dev:$version"
    fi
    echo
done
